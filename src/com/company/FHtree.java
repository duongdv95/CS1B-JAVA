public class FHtree<E> implements Cloneable {
    ...
    public int size(FHtreeNode<E> treeNode) {
        FHtreeNode<E> child;
        int count = 1;
        if (treeNode == null)
            return 0;
    ...
    public void collectGarbage(FHtreeNode<E> root) {
    ...
    public boolean softDeletedNodesFound() {
        return (size() != sizePhysical());
    }
    ...
    public FHtreeNode<E> addChild(FHtreeNode<E> treeNode, E x) {
        ...
        FHtreeNode<E> newNode = new FHtreeNode<E>(x,
                treeNode.firstChild, null, treeNode, mRoot, false);  // sb, chld, prv, rt, del
    ...
    public void  displayPhysical(FHtreeNode<E> treeNode, int level)
    {
        ...
        if (treeNode == null)
            return;
        if (treeNode.deleted)
            deleted = " (deleted node and node's children)";
        indent = blankString.substring(0, level);

        // pre-order processing done here ("visit")
        System.out.println( indent + treeNode.data + deleted) ;
